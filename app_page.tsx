 ...
 return <main style={{padding:'36px',maxWidth:'1320px',margin:'0 auto'}}>
   <section style={{display:'grid',gridTemplateColumns:'1.2fr 0.8fr',gap:'24px',alignItems:'stretch',marginBottom:'34px'}}>
     <div style={{background:'#fffaf2',padding:'56px',borderRadius:'30px',boxShadow:'0 12px 30px rgba(0,0,0,0.06)'}}>
      ...
      <div style={{display:'flex',gap:'12px',flexWrap:'wrap',marginBottom:'24px'}}>{categories.map((c,i)=><button key={i} onClick={()=>setCategory(c)} style={{background:colors[c]}}>{c}</button>)}</div>
   ...
   <section>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:'26px'}}>
      {filtered.map((recipe,i)=><div key={i}><RecipeCard recipe={recipe} onEdit={()=>editRecipe(i)}/><button style={{marginTop:'8px',background:'#b22222'}} onClick={()=>deleteRecipe(i)}>Delete</button></div>)}
    </div>
   </section>
   <ClaudeChat/>
 </main>
